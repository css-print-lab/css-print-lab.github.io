...
On each page, a page note area is created depending on the value of its `content` property. With the default CSS rules, it means that the area is created when a note whose identifier is `note` is displayed on the current page.
...
The note area is [blockified](https://drafts.csswg.org/css-display-4/#blockify) and added before the boxes generated for this page by the root element, so that the [page area](https://drafts.csswg.org/css-page-3/#page-model) acts as a container for the note area and the boxes generated by root element.
