...
- The **`@spread`** at-rule for spread-level layout declarations (gutter compensation, shared page box properties);
...
The `gutter: 10mm` declaration shifts the image 10mm toward the outer edge on each page, so the 20mm zone at the center of the spread (10mm on each side of the fold) carries no critical visual content.
...
- https://www.w3.org/TR/css-page-3/ → page placement + @spread 
- https://www.w3.org/TR/css-page-floats-3/ → spread float
